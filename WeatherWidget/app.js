...
let checkWeather = async (city)=>{
    ...
    if(response.status==200)
        {
            ...
            let tl = gsap.from(".widget,.widget h1,.widget h2,.footer,.container",{
                opacity : 0,
                duration : 1.6
            })
            tl.play();
    ...
    if(data.main.temp>0 && data.weather[0].main==="Clouds")
        {
    ...
    else if(data.main.temp>0 && data.weather[0].main==="Haze")
        {
            widgeticon.src = "./assets/cloudy.png";
            card.style.background = "#8C8C8C";
            contain.style.display = "inline";
            card.style.overflow = "hidden";
        }
    else if(data.main.temp>0 && data.weather[0].main==="Clear")
        {
    ...
    else if(data.main.temp<0 && data.weather[0].main==="Clouds")
        {
